#!/usr/bin/env python3
"""
Zombi Peak Performance Test

Measures maximum throughput and CPU usage using native HTTP load testing tools.
Requires: hey (brew install hey) or wrk (brew install wrk)

Usage:
    python peak_performance.py --url http://localhost:8080
    python peak_performance.py --url http://localhost:8080 --duration 60 --concurrency 200,500,1000
"""
...
@dataclass
class PerfResult:
    """Result from a single performance test run."""
    concurrency: int
    duration_secs: float
    total_requests: int
...
@dataclass
class PeakPerfReport:
    """Complete peak performance report."""
    url: str
    timestamp: str
    tool: str
...
def check_dependencies() -> Tuple[Optional[str], str]:
    ...
    if shutil.which("ab"):
        return "ab", "ab (Apache Bench) is available"
...
def get_zombi_pid() -> Optional[int]:
    """Get Zombi server PID."""
    try:
        # Use pgrep with exact match for the binary
        ...
        # Fallback: try finding by command line
        result = subprocess.run(
            ["ps", "-eo", "pid,comm"],
            capture_output=True, text=True
        )
...
def monitor_cpu(pid: int, interval: float = 0.2) -> Tuple[threading.Event, List[float]]:
    ...
    def _monitor():
        while not stop_event.is_set():
            try:
                # Use top for more accurate CPU sampling on macOS
                result = subprocess.run(
                    ["ps", "-p", str(pid), "-o", "%cpu"],
                    capture_output=True, text=True
                )
...
def parse_hey_output(output: str) -> PerfResult:
...
def parse_wrk_output(output: str) -> PerfResult:
...
def run_hey_test(
    ...
    cmd = [
        ...
        "-T", "application/json",
        "-d", payload,
        f"{url}/tables/peak-perf-{concurrency}"
    ]

    result = subprocess.run(cmd, capture_output=True, text=True)
    output = result.stdout + result.stderr

    perf_result = parse_hey_output(output)
...
def run_wrk_test(
    ...
    """Run wrk load test."""
    # Create Lua script for POST with payload
...
''')
    ...
    try:
        cmd = [
            "wrk",
            "-t", str(min(concurrency, 16)),  # wrk threads
            "-c", str(concurrency),
            "-d", f"{duration}s",
            "-s", lua_script,
            f"{url}/tables/peak-perf-{concurrency}"
        ...
        perf_result = parse_wrk_output(output)
        perf_result.concurrency = concurrency
...
def run_peak_test(
    ...
    """Run peak performance tests at multiple concurrency levels."""

    payload = json.dumps({
        "payload": "peak-performance-test-payload",
    ...
    # Get Zombi PID for CPU monitoring
    ...
    for concurrency in concurrency_levels:
        ...
        # Run test
        if tool == "hey":
            output, perf_result = run_hey_test(url, concurrency, duration, payload)
        else:
            output, perf_result = run_wrk_test(url, concurrency, duration, payload)
...
def print_report(report: PeakPerfReport):
    ...
    if report.server_stats_before and report.server_stats_after:
        ...
        print(f"   Server-side avg latency: {avg_latency:.1f} μs")

    print()
    print("=" * 70)


def save_report(report: PeakPerfReport, output_path: str):
...
def main():
    parser = argparse.ArgumentParser(
        description="Zombi Peak Performance Test",
...
Examples:
  ...
  python peak_performance.py --url http://localhost:8080 --concurrency 100,200,500,1000

  # Longer duration for more accurate results
  python peak_performance.py --url http://localhost:8080 --duration 60

  # Save results to file
  python peak_performance.py --url http://localhost:8080 --output peak_results.json
"""
    )
    ...
    parser.add_argument(
        ...
        help="Comma-separated concurrency levels (default: 50,100,200,500)"
    )
    parser.add_argument(
        "--duration",
        type=int,
    ...
    if args.output:
        ...
        save_report(report, "peak_performance_results.json")
